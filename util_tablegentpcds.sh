...
if [[ "$1" =~ ^[0-9]+$ && "$1" -gt "1" ]]; then
    ...
    if [[ "$2" == "orc" ]]; then
        ...
        timedate >> $CLOCK_FILE
        beeline -u "jdbc:hive2://`hostname -f`:10001/;transportMode=http" -i settingsTable.hql -f tpcds_dll/analyze.hql --hiveconf DB=tpcds_parquet_$INPUT_SCALE --hiveconf REDUCERS=$REDUCERS
        echo "End" >> $CLOCK_FILE
        timedate >> $CLOCK_FILE
        echo "" >> $CLOCK_FILE
    fi
