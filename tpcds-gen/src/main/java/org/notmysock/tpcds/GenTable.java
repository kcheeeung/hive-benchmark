...
public class GenTable extends Configured implements Tool {
  ...
  public int run(String[] args) throws Exception {
    ...
    URI link = new URI(dsuri.getScheme(), dsuri.getUserInfo(), dsuri.getHost(), dsuri.getPort(), dsuri.getPath(),
    ...
    Job job = Job.getInstance(conf, "GenTableTPCDS+" + table + "_" + scale);
  ...
  public Path genInput(String table, int scale, int parallel) throws Exception {
    ...
    for (int i = 1; i <= parallel; i++) {
      if (table.equals("all")) {
        out.writeBytes(
            String.format("./dsdgen -dir $DIR -force Y -scale %d -parallel %d -child %d\n", scale, parallel, i));
      } else {
