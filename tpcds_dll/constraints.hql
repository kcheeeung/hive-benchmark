...
alter table web_page add constraint ${DB}_wp_url foreign key  (wp_customer_sk) references customer (c_customer_sk) disable novalidate rely; --new
...
-- alter table customer add constraint ${DB}_c_lrd foreign key  (c_last_review_date_sk) references date_dim (d_date_sk) disable novalidate rely; -- for existing remove _sk
